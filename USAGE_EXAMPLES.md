...
- `parentheses` — removes parenthetical content **only if** it contains a number or `n.d` — so `(Smith et al., 2020)` and `(n.d.)` are removed, but `(see Figure 3)` is also removed because it contains a digit, while `(see discussion above)` is kept as-is
